...
const EditMessage = ({ messageId, setMessageId }: ComponentInputs) => {
  ...
  useEffect(() => {
    setFocus("messageContent", { shouldSelect: true });
  }, []);
  ...
  return (
    <div>
      ...
      <form className="flex justify-around" onSubmit={handleSubmit(onSubmit)}>
        <textarea
          className="px-4 py-2 w-96  border-violet-700 rounded-md border-2 border-zinc-800 bg-neutral-900 focus:outline-none "
